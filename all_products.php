...
<h3 style="margin-left: 1%; 
    ...
    <?php
    include_once './db_connector.php';
    $sql2 = "SELECT DISTINCT(product_category)  FROM category_table GROUP BY product_category ORDER BY product_id DESC";
...
#select_by_category:hover{
   ...
   foreach ($rows as $row) { 
    ...
    <input type="hidden" name="product_id" value="<?php echo $row->product_id; ?>" />
     <button onclick="return confirm('Are you sure you want to delete this product?');" style="display:none">Delete</button>
   </form>      
   </div>
    </div>
   <?php } ?>   
